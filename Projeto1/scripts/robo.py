...
#from aruco import roda_todo_frame, scaneou
...
class Follower:
    ...
    def __init__(self, cor, id, imagem):
	    ...
	    #self.image_sub = rospy.Subscriber('/camera/color/image_raw/compressed_mouse_left',
        #topico da camera do robo simulado('/camera/image/compressed')
...
#     #     return self.laser_msg.ranges[pos]
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            if self.estado == 2:
                #cv_image,ids,corners = roda_todo_frame(cv_image)
                #print(corners)
                pass
    ...
    def segue_linha(self):
        ...
        self.twist.angular.z = -float(err) / 1100
        min_dist = inf 
        if self.laser_msg.ranges is not None and len(self.laser_msg.ranges) != 0:
            dists = [self.laser_msg.ranges[a] for a in range(-50,51,1) if a !=0 and self.laser_msg.ranges[a]!=0]
            try:
                min_dist = min(dists)
            except:
                min_dist = inf
        if min_dist < 0.4:
            return "Slalom"
        elif self.odom_distance() < 1.0 and (rospy.Time.now() - self.time_ini > rospy.Duration(30)):
            return "Procurando creeper"
        return "Seguindo linha"

    def slalom(self):
        dists = {a:self.laser_msg.ranges[a] for a in range(-50,51,1) if a !=0 and self.laser_msg.ranges[a]!=0}
        min_a = min(dists,key=dists.get)
        min_dist = dists[min_a]
        lado = min_a/abs(min_a)
        ...
        z1 = (min_dist * lado)*0.9
        print(z1)

        #if ind[0] == 2: 
        #    z1 = -z1
        self.twist.angular.z = -float(err/1300 + z1)
        dists = [el > 0.4 for el in (self.laser_msg.ranges[0:15] + self.laser_msg.ranges[344:359])]
        cond = dists.count(True) > 5 
        if cond:
            return "Seguindo linha"
        elif self.odom_distance() < 1.65 and (rospy.Time.now() - self.time_ini > rospy.Duration(30)):
            return "Procurando creeper"
        else:
            return "Slalom"
    
    def procura_creeper(self):
        m_colors={'pink':self.size_pink,'blue':self.size_blue,'green':self.size_green}
        m_color_escolhida = cv2.moments(m_colors[self.cor])
        err = 0
        if m_color_escolhida['m00'] > 0:
            cx = int(m_color_escolhida['m10']/m_color_escolhida['m00'])
            err = cx - self.w/2
