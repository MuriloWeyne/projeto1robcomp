...
#     #     return self.laser_msg.ranges[pos]
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            mask = cv2.inRange(hsv, lower_yellow, upper_yellow) # mask_yellow
            lower_white = np.array([0, 0, 200],dtype=np.uint8)
            upper_white = np.array([180, 5, 255],dtype=np.uint8)
    ...
    def segue_linha(self):
        ...
        if any([ el < 0.4 for el in (self.laser_msg.ranges[0:30] + self.laser_msg.ranges[329:359])]):
            return "Slalom"
        elif self.odom_distance() < 1.64 and (rospy.Time.now() - self.time_ini > rospy.Duration(60)):
    ...
    def slalom(self):
        ...
        if ind[0] == 2: 
            z1 = -z1
        self.twist.angular.z = -float(err/800 + z1)
        if all([ el > 0.4 for el in (self.laser_msg.ranges[0:30] + self.laser_msg.ranges[329:359])]):
            return "Seguindo linha"
        elif self.odom_distance() < 1.65 and (rospy.Time.now() - self.time_ini > rospy.Duration(60)):
            return "Procurando creeper"
        else:
            return "Slalom"
    ...
    def procura_creeper(self):
        m_azul = cv2.moments(self.size_blue)
        err = 0
        if m_azul['m00'] > 0:
