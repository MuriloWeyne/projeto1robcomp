...
def roda_todo_frame(imagem):
	...
	centros = None
	try:
		#imagem = cv2.flip(imagem, -1) # Descomente se for robo real
		#imagem = bridge.imgmsg_to_cv2(imagem, "bgr8") 			# imagem não compressed
		#imagem = cv2.resize(imagem,(imagem.shape[1]*2,imagem.shape[0]*2)) # resize image se necessario
		...
		if ids is not None:
			...
			aruco.drawAxis(imagem, camera_matrix, camera_distortion, rvec, tvec, 1)

			#-- Print tvec vetor de tanslação em x y z
			str_position = "Marker x=%4.0f  y=%4.0f  z=%4.0f"%(tvec[0], tvec[1], tvec[2])
			print(str_position)
			cv2.putText(imagem, str_position, (0, 100), font, 1, (0, 255, 0), 1, cv2.LINE_AA)

			##############----- Referencia dos Eixos------###########################
			# Linha referencia em X
			cv2.line(imagem, (imagem.shape[1]//2,imagem.shape[0]//2), ((imagem.shape[1]//2 + 50),(imagem.shape[0]//2)), (0,0,255), 5) 
			# Linha referencia em Y
			cv2.line(imagem, (imagem.shape[1]//2,imagem.shape[0]//2), (imagem.shape[1]//2,(imagem.shape[0]//2 + 50)), (0,255,0), 5) 	
			...
			for i,j in zip(range(4),range(4,8)): imagem = cv2.line(imagem, tuple(imgpts[i]), tuple(imgpts[j]),(0,0,255),4);
			imagem = cv2.drawContours(imagem, [imgpts[4:]],-1,(0,0,255),4)
			centros =[]
			...
			for corner in corners[0]:
				...
				cv2.circle(imagem,(x_centro,y_centro),7, (255,0,0),thickness=3)
			
		
		return imagem,ids,centros
